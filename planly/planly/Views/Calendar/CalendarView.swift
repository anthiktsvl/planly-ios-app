                ...
                VStack(spacing: 0) {
                    ...
                    HStack {
                        ...
                        Text(currentMonth.formatted(.dateTime.month(.wide).year()))
                            .adaptiveFont(.title2, weight: .bold)
        ...
        Button(action: action) {
            VStack(spacing: 4) {
                Text("\(Calendar.current.component(.day, from: date))")
                    .adaptiveFont(.subheadline, weight: isToday ? .bold : .regular)
                    .foregroundColor(
                        isSelected ? .black :
                        isToday ? ColorTheme.light(for: themeManager.currentTheme) :
                        .primary
                    )
                
                if hasTask {
                    Circle()
                        .fill(isSelected ? Color.black : ColorTheme.primary(for: themeManager.currentTheme))
            ...
            .background(
                Group {
                    if isSelected {
                        ...
                        ColorTheme.dark(for: themeManager.currentTheme).opacity(0.1)
                    } else {
                        Color.clear
                    }
                }
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 8)
                    .stroke(isToday && !isSelected ? ColorTheme.primary(for: themeManager.currentTheme) : Color.clear, lineWidth: 1)
            )
        }
        .padding(2)
    }
        ...
        HStack(spacing: 12) {
            ...
            Button {
                ...
                Image(systemName: task.isCompleted ? "checkmark.circle.fill" : "circle")
                    .font(.title3)
                    .foregroundColor(task.isCompleted ? .green : ColorTheme.primary(for: themeManager.currentTheme))
            }
